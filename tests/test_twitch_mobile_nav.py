...
def test_twitch_homepage_failing(web_driver):
...
def test_twitch_mobile_nav(mobile_driver):
    home = HomePage(mobile_driver)
    search = SearchPage(mobile_driver)
    results = SearchResultsPage(mobile_driver)
    ...
    current_url = home.get_current_url()
    assert "?desktop-redirect=true" in current_url
    ...
    assert "/directory/category/starcraft-ii" in results.get_current_url()
